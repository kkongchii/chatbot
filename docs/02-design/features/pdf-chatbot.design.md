...
`server.js` 단일 파일 내에서 `loadPdfText()`, `askOpenAI()` 함수를 명확히 분리한다. 별도 `lib/` 디렉토리 없이 Starter 수준을 유지하면서도, 각 관심사(PDF 파싱 / OpenAI 호출 / 라우팅)를 함수 단위로 분리해 가독성을 확보한다.
...
├── package.json
├── vercel.json
├── .env                   # OPENAI_API_KEY, PORT (Git 제외)
└── .gitignore
```
...
├── loadPdfText()     PDF 파일 읽기 → pdf-parse → 텍스트 반환 (async)
├── askOpenAI()       OpenAI ChatCompletion 호출 → 답변 문자열 반환 (async)
...
```js
async function askOpenAI(pdfText, userMessage) {
  ...
  const response = await client.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: `당신은 근로기준법 전문 도우미입니다. 아래 근로기준법 내용을 바탕으로 질문에 답변하세요.\n\n${pdfText}`
      },
...
2. askOpenAI(pdfText, message) 호출
3. 성공: res.json({ reply })
...
<body>
  <div class="container">
    <h1>근로기준법 챗봇</h1>
    <div id="chatMessages">      <!-- 메시지 목록 스크롤 영역 -->
      <!-- .message.user / .message.bot 동적 추가 -->
    </div>
    <div class="inputArea">
...
| `userInput` keydown Enter | `sendMessage()` 호출 |
...
| `OPENAI_API_KEY` 미설정 | `askOpenAI()` 내 500 에러 | "답변을 가져오는 데 실패했습니다." |
| OpenAI API 타임아웃 / 오류 | try/catch → 500 응답 | "답변을 가져오는 데 실패했습니다." |
...
- [ ] `OPENAI_API_KEY`는 `server.js`의 `process.env`에서만 참조
- [ ] `public/index.html`에 API 키 문자열 없음
- [ ] `.env` 파일 `.gitignore`에 포함됨
- [ ] PDF 파싱은 `server.js`에서만 수행 (프론트엔드에서 직접 접근 불가)

---

## 10. Vercel 배포 체크리스트

- [ ] Vercel 대시보드 → Settings → Environment Variables에 `OPENAI_API_KEY` 설정
- [ ] `vercel.json`의 `includeFiles`에 `docs/**` 포함 확인
- [ ] `vercel deploy` 후 공개 URL에서 T-01 시나리오 테스트
